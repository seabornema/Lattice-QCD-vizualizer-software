...
const std::array<std::array<int,2>,12> EdgeVertexIndices = {{
	{0, 1},
	{1, 3},
	{2, 3},
	{0, 2},
...
const std::array<int,256> EdgeMasks = {
...
const std::array<std::array<int,16>,256> TriangleTable = {{
	{ -1 },
	{ 0, 3, 8, -1 },
	{ 0, 9, 1, -1 },
	{ 3, 8, 1, 1, 8, 9, -1 },
...
uint8_t find_cube_index(const std::array<double,8>& corners, double isovalue) {
    ...
    for (int i = 0; i < 8; ++i) {
        if (corners[i] > isovalue)
            cubeIndex |= (1 << i);  //this sets bit number i to 1. bit manipulation??????
    }
    return cubeIndex;
}
...
//below function return the position for a single vertex based off of the edge index
std::vector<GLfloat> coord_from_edge(int edgeIndex, GLfloat x0, GLfloat y0, GLfloat z0, GLfloat size,
                                     GLfloat a, GLfloat b, GLfloat isovalue) {
    GLfloat t = lerp_pos(a, b, isovalue);
    switch(edgeIndex) {
        case 0: return {x0 + t*size, y0, z0};               // edge 0: v0-v1, along x
        case 1: return {x0 + size, y0 + t*size, z0};        // edge 1: v1-v2, along y
        case 2: return {x0 + (t)*size, y0 + size, z0};    // edge 2: v2-v3, along x reversed
        case 3: return {x0, y0 + (t)*size, z0};           // edge 3: v3-v0, along y reversed
        case 4: return {x0 + t*size, y0, z0 + size};       // edge 4: v4-v5, along x
        case 5: return {x0 + size, y0 + t*size, z0 + size}; // edge 5: v5-v6, along y
        case 6: return {x0 + (t)*size, y0 + size, z0 + size}; // edge 6: v6-v7, along x reversed
        case 7: return {x0, y0 + (t)*size, z0 + size};    // edge 7: v7-v4, along y reversed
        case 8: return {x0, y0, z0 + t*size};               // edge 8: v0-v4, along z
        case 9: return {x0 + size, y0 , z0+ t*size};       // edge 9: v1-v5, along z
        case 10: return {x0 + size, y0 + size, z0 + t*size}; // edge10: v3-v7, along z
        case 11: return {x0, y0 + size, z0 + t*size};      // edge11: v2-v6, along z
        default: return {};
    }
}
std::vector<GLfloat> calculate_normal(std::vector<GLfloat> points) {
    GLfloat x1 = points[3] - points[0];
    GLfloat y1 = points[4] - points[1]; 
    GLfloat z1 = points[5] - points[2];

    GLfloat x2 = points[6] - points[0];
    GLfloat y2 = points[7] - points[1]; 
    GLfloat z2 = points[8] - points[2];

    GLfloat nx = y1 * z2 - z1 * y2;
    GLfloat ny = z1 * x2 - x1 * z2;
    GLfloat nz = x1 * y2 - y1 * x2;
/*
    GLfloat length = std::sqrt(nx * nx + ny * ny + nz * nz);
        nx /= length;
        ny /= length;
        nz /= length;
*/
...
std::vector<GLfloat> triangles_from_cube(const std::array<double, 8>& corners,double isovalue,GLfloat x0, GLfloat y0, GLfloat z0,GLfloat size,std::vector<GLfloat> color,std::vector<GLfloat> fade_color) {
	...
	std::array<int, 16> guys = find_edge_set(find_cube_index(corners, isovalue));

    std::vector<GLfloat> triangles;
	bool shouldbreak = false;
    for (int i = 0; i < 5; i++) {
        std::vector<GLfloat> curr_pos;
        for (int j = 0; j < 3; j++) {
            int index = (3 * i) + j;
            if (guys[index] == -1) {
			shouldbreak = true;
			...
			std::array<int,2> edge_set= EdgeVertexIndices[guys[index]];
			GLfloat a = (GLfloat)corners[edge_set[0]];
			GLfloat b = (GLfloat)corners[edge_set[1]];

            std::vector<GLfloat> holder = coord_from_edge(guys[index], x0, y0, z0, size,a,b,isovalue);
            curr_pos.insert(curr_pos.end(), holder.begin(), holder.end());
        	} 
			if(shouldbreak == true){
				break;
			}
        std::vector<GLfloat> normal = calculate_normal(curr_pos);

        for (int v = 0; v < 3; v++) {
            triangles.push_back(curr_pos[v * 3 + 0]);
            triangles.push_back(curr_pos[v * 3 + 1]);
            triangles.push_back(curr_pos[v * 3 + 2]);

			triangles.insert(triangles.end(), color.begin(), color.end());
			triangles.insert(triangles.end(), fade_color.begin(), fade_color.end());

            triangles.push_back(normal[0]);
            triangles.push_back(normal[1]);
            triangles.push_back(normal[2]);
...
std::vector<GLfloat> marching_builder(std::vector<std::vector<std::vector<double>>> lattice, double isolevel,GLfloat lattice_size,std::vector<GLfloat> color,std::vector<GLfloat> fade_color) {
    int N = lattice.size()-1;
    std::vector<GLfloat> temp;
	GLfloat cellsize = lattice_size/((double)N);
	#pragma omp for collapse(3)
    for(int i =0; i < N; i++){
        for(int j =0; j < N; j++){
            for(int k =0; k < N; k++){
                GLfloat x = (GLfloat)(i*cellsize);
                GLfloat y = (GLfloat)(j*cellsize);
                GLfloat z = (GLfloat)(k*cellsize);
                std::array<double,8> corners = {lattice[i][j][k],lattice[i+1][j][k],lattice[i][j+1][k],lattice[i+1][j+1][k],lattice[i][j][k+1],lattice[i+1][j][k+1],lattice[i][j+1][k+1],lattice[i+1][j+1][k+1]};
                std::vector<GLfloat> current = triangles_from_cube(corners,isolevel,x,y,z,cellsize,color,fade_color);
...
std::vector<GLuint> index_generator(int size){
    std::vector<GLuint> temp;
	int truesize = size/12;
	int interator_size = size - 9*truesize;
    for(int i =0; i <interator_size;i++)
    {
        temp.push_back((GLuint)i);
    }
	return temp;
}