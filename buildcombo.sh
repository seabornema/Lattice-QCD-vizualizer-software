...
fixed_variable beta_fixed("beta",&beta,0);
variable_set GUI({iso_fixed,iso_speed_fixed,beta_fixed},iso_fixed);
...
Eigen::Matrix2cd field_strength(const std::vector<std::vector<std::vector<std::vector<Links>>>>& lattice,const std::array<int,4>& position, int mu,int nu) {
    int x = position[0];
    int y = position[1];
    int z = position[2];
    int t = position[3];
    ...
    Eigen::Matrix2cd U1 = plaquette(lattice,x,y,z,t,mu,nu);
    Eigen::Matrix2cd U2 = -lattice[x][y][z][t].U[mu]*lattice[tmod(x+mud[0]-nud[0],L)][tmod(y+mud[1]-nud[1],L)][tmod(z+mud[2]-nud[2],L)][tmod(t+mud[3]-nud[3],Nt)].U[nu].adjoint()*lattice[tmod(x-nud[0],L)][tmod(y-nud[1],L)][tmod(z-nud[2],L)][tmod(t-nud[3],Nt)].U[mu].adjoint()*lattice[tmod(x-nud[0],L)][tmod(y-nud[1],L)][tmod(z-nud[2],L)][tmod(t-nud[3],Nt)].U[nu];
    Eigen::Matrix2cd U3 =  lattice[x][y][z][t].U[nu]*lattice[tmod(x-mud[0]+nud[0],L)][tmod(y-mud[1]+nud[1],L)][tmod(z-mud[2]+nud[2],L)][tmod(t-mud[3]+nud[3],Nt)].U[mu].adjoint()*lattice[tmod(x-mud[0],L)][tmod(y-mud[1],L)][tmod(z-mud[2],L)][tmod(t-mud[3],Nt)].U[nu].adjoint()*lattice[tmod(x-mud[0],L)][tmod(y-mud[1],L)][tmod(z-mud[2],L)][tmod(t-mud[3],Nt)].U[mu];
    Eigen::Matrix2cd U4 = -lattice[tmod(x-nud[0],L)][tmod(y-nud[1],L)][tmod(z-nud[2],L)][tmod(t-nud[3],Nt)].U[nu].adjoint()*lattice[tmod(x-mud[0]-nud[0],L)][tmod(y-mud[1]-nud[1],L)][tmod(z-mud[2]-nud[2],L)][tmod(t-mud[3]-nud[3],Nt)].U[mu].adjoint()*lattice[tmod(x-mud[0]-nud[0],L)][tmod(y-mud[1]-nud[1],L)][tmod(z-mud[2]-nud[2],L)][tmod(t-mud[3]-nud[3],Nt)].U[nu]*lattice[tmod(x-mud[0],L)][tmod(y-mud[1],L)][tmod(z-mud[2],L)][tmod(t-mud[3],Nt)].U[mu];
    return U1+U2+U3+U4;
}
...
double topological_single(const std::vector<std::vector<std::vector<std::vector<Links>>>>& lattice,const std::array<int,4> pos) {
    ...
    do{
        sum += levi_c(starter)*(field_strength(lattice,pos,starter[0],starter[1])*field_strength(lattice,pos,starter[2],starter[3])).trace().real();
...
std::vector<GLfloat> polyakov_colo = {0.93333f,0.0647f,0.023523f};
std::vector<GLfloat> topological_charge_color = {0.053523f,0.1647f,0.88333f};
//std::vector<GLfloat> lattice_fade_color = {0.9643f,0.4647f,0.3843f};
std::vector<GLfloat> lattice_fade_color = {0.3843f,0.4647f,0.8643f};
...
void regenerate_triangles(double isolevel,const std::vector<std::vector<std::vector<double>>>& lattice,std::vector<GLfloat>& vertices,std::vector<GLuint>& indices,const VBO& VBO1,const EBO& EBO1,std::vector<GLfloat> lattice_color){
...
void animate(const std::vector<std::vector<std::vector<std::vector<Links>>>>& lattice,double iso,std::vector<GLfloat>& vertices,std::vector<GLuint>& indices,const VBO& VBO1,const EBO& EBO1,int index) {
    regenerate_triangles(iso,topological_charge(lattice,index%Nt),vertices,indices,VBO1,EBO1,topological_charge_color);
...
std::vector<std::vector<std::vector<double>>> sphere_example(int n){
      std::vector<std::vector<std::vector<double>>> temp(n, std::vector<std::vector<double>>(n, std::vector<double>(n, 0.0))); 
    for(int i =0;i<n;i++){
        for(int j =0;j<n;j++){
            for(int k =0;k<n;k++){
        temp[i][j][k] = (i-n/2)*(i-n/2)+(j-n/2)*(j-n/2) + (k-n/2)*(k-n/2);
    }   
    }    
    }
    return temp;
...
int main() {
    ...
    cold_start(lattice);
    full_sweep(lattice,100,beta);
 //   action_minimizing_sweep(lattice,20);
   temp = Polyakov_loop_real(lattice);
  //  temp = sphere_example(100);
    

    int N = temp.size();
 //   initalize();
    
    vertices =  marching_builder(temp,iso,N,polyakov_colo,lattice_fade_color);
    indices  = index_generator(vertices.size());
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if(glfwGetKey(window, GLFW_KEY_R) == GLFW_PRESS){
            ...
            *GUI.current_guy.value *= 1.1;
             ...
             regenerate_triangles(iso,temp,vertices,indices,VBO1,EBO1,polyakov_colo);
        ...
        if(glfwGetKey(window, GLFW_KEY_T) == GLFW_PRESS){
            ...
            regenerate_triangles(iso,temp,vertices,indices,VBO1,EBO1,topological_charge_color);
        }
